...
void onMouse(int who, int state, int x, int y)
{
	...
	switch (who)
	{
		...
		case GLUT_WHEEL_UP:
			...
			else if (mode==3)//scaling
			{
				models[cur_idx].scale.z += 0.025;
			}
			else if (mode==2)//rotstion
			{
				models[cur_idx].rotation.z += (PI / 180.0) * 5;
			}
			else if (mode==4)//center translate mode
			{
				main_camera.center.z += 0.02f;
				//setViewingMatrix();
				printf("Camera Viewing Direction = ( %f , %f , %f )\n", main_camera.center.x, main_camera.center.y, main_camera.center.z);
			}
			else if (mode == 5)//eye translate mode
			{
				main_camera.position.z -= 0.02f;
				//setViewingMatrix();
...
void onMouseMotion(int x, int y)
{
	...
	{
		main_camera.center.x += diff_x * (1.0 / 8000.0);
		main_camera.center.y += diff_y * (1.0 / 8000.0);
		//setViewingMatrix();
		printf("Camera Viewing Direction = ( %f , %f , %f )\n", main_camera.center.x, main_camera.center.y, main_camera.center.z);
	}
	else if (mode == 5)//eye translate mode
...
{
	printf("Information :\n");
	printf("mode position : x = %f , y = %f , z = %f\n", models->position.x, models->position.y, models->position.z);
	printf("mode rotation : x = %f , y = %f , z = %f\n", models->rotation.x, models->rotation.y, models->rotation.z);
	printf("mode scaling : x = %f , y = %f , z = %f\n", models->scale.x, models->scale.y, models->scale.z);
	printf("camera position : x = %f , y = %f , z = %f\n", main_camera.position.x, main_camera.position.y, main_camera.position.z);
	printf("center position : x = %f , y = %f , z = %f\n", main_camera.center.x, main_camera.center.y, main_camera.center.z);
	if (projection)
	{
		printf("projection : orthogonal projection\n");
	}
	else
	{
		printf("projection : perspective projection\n");
	}

	if (mode == 1)
	{
		printf("you are in object translate mode\n");
	}
	else if (mode == 2)
	{
		printf("you are in object rotation mode\n");
	}
	else if (mode == 3)
	{
		printf("you are in object scaling mode\n");
	}
	else if (mode == 4)
	{
		printf("you are in center translate mode\n");
	}
	else if (mode == 5)
	{
		printf("you are in eye translate mode\n");
	}
...
void onKeyboard(unsigned char key, int x, int y)
{
	...
	case 'c':
		mode = 4;
		printf("c : go to center translate mode\n");
		break;
	case 'C':
		mode = 4;
		printf("C : go to center translate mode\n");
	...
	case 'I':
		printfInformation();
		break;
	case 't':  //Geometrical-->transformation
		mode = 1;
	...
	case 's':
		mode = 3;
		printf("s : go to object scaling mode\n");
		break;
	case 'S':
		mode = 3;
		printf("S : go to object scaling mode\n");
	...
	case 'r':
		mode = 2;
		printf("r : go to object rotation mode\n");
		break;
	case 'R':
		mode = 2;
		printf("R : go to object rotation mode\n");
